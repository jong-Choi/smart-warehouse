...
import { fetchSalesOverview, fetchLocationSales } from "@/api/salesApi";
...
export function DashboardSalesOverviewPage() {
  ...
  const [overviewData, setOverviewData] = useState<SalesOverviewData | null>(
    null
  );
  const [locationData, setLocationData] = useState<LocationSalesData[]>([]);
  ...
  const loadData = async (year: number) => {
    try {
      ...
      const [overviewResponse, locationResponse] = await Promise.all([
        fetchSalesOverview(year),
        fetchLocationSales(year),
      ]);
        ...
        <Card>
          <CardContent className="p-6">
            <div className="text-center text-red-600">
              ...
              <Button onClick={() => loadData(currentYear)} className="mt-4">
                다시 시도
              </Button>
            </div>
          </CardContent>
